...
void* BasicHashtableImpl::allocateBuckets(size_t count) const {
    ...
    LOG_FATAL_IF((sb == nullptr), "Could not allocate %u bytes for hashtable with %u buckets.",
...
void BasicHashtableImpl::determineCapacity(size_t minimumCapacity, float loadFactor,
	...
	LOG_FATAL_IF((loadFactor <= 0.0f || loadFactor > 1.0f),
    ...
    count = PRIMES[i];
    LOG_FATAL_IF(count == 0, "Could not determine required number of buckets for "
